...
ServoTimer2::ServoTimer2() {
  if(ChannelCount < NBR_CHANNELS) {
    ...
    chanIndex = 0;  // todo: too many channels, assigning 0 inhibits this instance from functioning
  }
}
...
uint16_t ServoTimer2::read() const {
  if(chanIndex > 0) {
...
void ServoTimer2::write(const uint16_t pulsewidth) {
